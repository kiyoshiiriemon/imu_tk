...
int main(int argc, char** argv)
{
  ...
  cout<<"Importing IMU data from the Matlab matrix file : "<< argv[1]<<endl;  
  importAsciiData( argv[3], acc_data_target, imu_tk::TIMESTAMP_UNIT_SEC );
  cout<<"Importing IMU data from the Matlab matrix file : "<< argv[2]<<endl;  
  importAsciiData( argv[4], gyro_data_target, imu_tk::TIMESTAMP_UNIT_SEC  );
...
//   cout<<"Read "<<acc_data.size()<<" tuples"<<endl;
  ...
  for( int i = 0; i < n_samps; i++) {
    std::cout << ca[i].x() << " " << ca[i].y() << " " << ca[i].z() << " "
              << cg[i].x() << " " << cg[i].y() << " " << cg[i].z() << std::endl;
  }
